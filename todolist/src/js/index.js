(function main() {
  ...
  const getToDoList = (item) => {
    ...
    //   const toDoListState = div.querySelector('.Todo_List_State');
    //   toDoListState.checked = data[findIdx(data, id)].state;
    //   if (data[id].state) {
    ...
    div.addEventListener('click', (e) => {
      if (e.target.tagName === 'BUTTON') return;
      const index = findIdx(data, id);
      toggle(id);
      const { state } = data[index];
      const checkBoxState = div.querySelector('.Todo_List_State');
      checkBoxState.checked = state;

      div.classList.toggle('complete', state);
    });
    ...
    delBTN.className = 'ToDo_list_delBtn btn';
    delBTN.type = 'button';
